...
5. **Staff tree stays off the public site** (policy per HANDOFF §6: staff docs move to Google Docs; the site becomes user-facing only). The conversion script excludes — content, assets, and binaries — `site/docs/IT/`, `site/docs/Networking/`, and `site/docs/FDM Printers/Dual Head FDM Printer/Operations & Safety Manual/Raise3D E2 Printer Staff Service Manual.md`; the `IT` and `Networking` sidebar groups go away. Public pages that link into the excluded trees are reported by the script for later repointing at the Google Docs homes. ⚠️ Known casualty: `Networking/Learning Assignment/` (student-facing IOT assignment) goes dark with its parent tree — owner to relocate it if it should stay public.
