...
- **The manual comes first in its sidebar group.** Starlight orders an autogenerated
  group alphabetically, which buries the manual under anything earlier in the alphabet
  (an `Activities/` folder, a "CAM Guide"). Pin it with `sidebar.order` in the
  frontmatter so the first thing a student sees in a machine's group is how to run the
  machine:
  ...
  Everything else in the group keeps sorting alphabetically after it.
...
2. Pin the manual first in its sidebar group with `sidebar.order: 1`.
3. Strip Google Docs export artifacts: stray `##` on non-headings, bold-as-heading lines, empty headings, escaped characters, redundant title repetition, stale frontmatter keys.
4. Merge the machine's safety manual into the machine page as callouts per the rules above; confirm every safety item is either in the top hazards block or inline, then delete the safety manual file.
5. Rewrite sections to the structure above; move "not for" content to the which-machine page.
6. Add `public/_redirects` entries for the retired operation- and safety-manual URLs → the merged page.
7. Check links: every internal link uses the `/docs/…/` URL form and resolves on the built site.
8. Build locally if you can (`npm ci && npx astro build`) — it catches broken images and malformed frontmatter. If you're editing from the GitHub web editor, the PR's Cloudflare build check does the same job.
