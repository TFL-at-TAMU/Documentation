...
- **Lasers** get exactly one callout, and it **states the laser class**. A dangerous laser (Class 3R/3B/**4**) gets the top `:::danger` callout (training + goggles); an eye-safe **Class 1/2** laser gets a `> [!NOTE]` instead (e.g. the 3D scanner: "Class 1 — eye-safe by design, no precautions needed"). Either way, all laser-safety information consolidates to that one callout — don't scatter laser mentions (training, goggles, "the laser head moves") through the intro, Before you start, PPE, or Operating. On a machine where the laser is the primary function (the laser cutter), the laser is naturally the subject throughout the page, but its *safety* still consolidates to that one callout. On a machine where the laser is a secondary feature (e.g. the CNC's laser module), keep it out of the body entirely — the single top callout is the only place it appears.
