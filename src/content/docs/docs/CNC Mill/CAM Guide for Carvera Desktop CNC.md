---
title: "CAM Guide for Carvera Desktop CNC"
source_id: "1mMPYvKm1WcPMXqxKzWPTf_exPH9mU961AeBnq2X-Pxk"
modified: "2026-04-12T20:35:31.127Z"
---

Fusion CAM Guide for Carvera CNC

Machine Name: Carvera Desktop CNC

Location: The Fab Lab

Version: v1.1

Last Updated: 4/12/2026

Responsible Student Worker: Marcus Gou, Aiden Stewart

Linked Machine Manual: [CNC Mill (Makera Carvera)](/docs/cnc-mill/cnc-mill-makera-carvera/)

Pasted info removed from main doc:

  * Be prepared to adjust the feeds and speeds based on the machine and tool specifications. Information can be found on the [wiki](<https://www.google.com/url?q=http://wiki.makera.com/en/speeds-and-feeds&sa=D&source=editors&ust=1776804212022155&usg=AOvVaw3NVCvbgQ2krHjJRD_PBPGh>) or in the Carvera discord.


  * For Fusion, only tool number (Tool library > Post Processing > Tool Number) matters. The bit length offset is measured by the Carvera during every tool change.
  * The required software is installed:


  *   * Fusion360 plugins for the Carvera machine.
  * The relevant Fusion360 tool libraries or user-made tools for the job.



## 1\. What This Machine Is For

Use this machine to:

  * [PRIMARY USE CASE]
  * [SECONDARY USE CASE]
  * [OPTIONAL ADDITIONAL USE]



## 2\. What This Machine Is Not For

Do not use this machine for:

  * [CLEAR NON-USE / LIMITATION]
  * [CLEAR NON-USE / LIMITATION]
  * [OPTIONAL LIMITATION]



## 3\. What You Need Before You Start

Before operating this machine, ensure:

  * [REQUIRED CONDITION – e.g., staff present]
  * [REQUIRED CONDITION – e.g., safety manual acknowledged]
  * [REQUIRED PPE, if any]
  * [REQUIRED FILE TYPE / MATERIAL / PREP STEP]



## 4\. Machine Overview

[INSERT IMAGE OR DIAGRAM OF MACHINE]

Label the following (if applicable):

  * [MAIN CONTROL INTERFACE]
  * [EMERGENCY STOP LOCATION]
  * [KEY USER INTERACTION POINTS]



## 5\. Basic Operating Workflow

### 5.1 Start-Up

  1. [STEP 1 – POWER ON / INITIAL ACTION]
  2. [STEP 2 – SOFTWARE / INTERFACE ACCESS]
  3. [STEP 3 – BASIC READINESS CHECK (USER-LEVEL ONLY)]



### 5.2 Running a Job

  1. [LOAD FILE / MATERIAL]
  2. [SET ESSENTIAL PARAMETERS ONLY]
  3. [START JOB]
  4. During operation, confirm that:


  * [WHAT NORMAL OPERATION LOOKS LIKE]



### 5.3 End-of-Job / Shutdown

  1. [STOP OR COMPLETE JOB]
  2. [REMOVE PART / MATERIAL SAFELY]
  3. [REQUIRED USER CLEANUP]
  4. [RESET / POWER DOWN IF REQUIRED]



## 6\. User Responsibilities After Use

After using this machine, you are responsible for:

  * [CLEANING ACTION]
  * [RETURNING TOOLS / MATERIALS]
  * [REPORTING ISSUES OR ABNORMAL BEHAVIOR]
  * [LOGGING USAGE, IF APPLICABLE]



## 7\. Stop Conditions

Stop immediately and notify Prototyping Studio staff if:

  * [ABNORMAL NOISE / MOTION]
  * [SMOKE, SPARKS, LEAKS, OR ODORS]
  * [UNEXPECTED MACHINE BEHAVIOR]
  * [YOU ARE UNSURE HOW TO PROCEED]



Do not attempt to troubleshoot major issues yourself.

## 8\. Common Issues & What To Do (Optional)

  * Issue: [COMMON, MINOR ISSUE]  
Action: [SIMPLE ACTION – often “Notify staff”]
  * Issue: [COMMON, MINOR ISSUE]  
Action: [SIMPLE ACTION]



## 9\. External Resources

For more detailed information, refer to:

  * [MANUFACTURER USER MANUAL LINK]
  * [OFFICIAL VIDEO TUTORIAL LINK]
  * [ADVANCED OR TROUBLESHOOTING DOCUMENTATION]



## 10\. Questions or Help

If you have questions or need assistance at any point, ask a Fab Lab staff member. Staff are always present during operating hours.

* * *

End of Operations Manual